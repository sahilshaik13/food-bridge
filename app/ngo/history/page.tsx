...
function HistoryContent() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        setError("Failed to load NGO history from backend.");
        setItems([]);
      }
    };
    run();
  ...
  useEffect(() => {
    ...
    const historyRef = ref(realtimeDatabase, `history_feeds/emergency/ngo/${profile.entity_id}`);
  ...
  return (
    <main className="min-h-screen">
      <PageHeader eyebrow="NGO coordinator" title="Pickup History" description="Rejected, completed, and expired pickup records." />
      ...
      <section className="mx-auto max-w-6xl px-5 py-6">
        {emergencyHistory.length > 0 && (
          <div className="mb-6 overflow-hidden rounded-2xl border border-ink/10 bg-white shadow-lift">
            <div className="border-b border-ink/10 bg-field px-4 py-3 text-sm font-bold text-ink">Emergency Request History (Realtime)</div>
            <div className="divide-y divide-ink/5">
              {emergencyHistory.map((item: any) => (
                <div key={item.id} className="px-4 py-3 text-sm">
                  <p className="font-bold capitalize text-ink">{item.food_type} · {item.status}</p>
                  <p className="text-ink/60">{item.pledged_kg}/{item.quantity_goal_kg} kg · Urgency: {item.urgency_level || "high"} · {new Date(item.updated_at || item.created_at).toLocaleString()}</p>
                </div>
              ))}
            </div>
          </div>
